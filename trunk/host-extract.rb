    ...
    h1.each do |l|        
        ...
        if l =~ /#{needle}/mi
            if l.index("#{needle}").to_i > 12
                sn = l[l.index("#{needle}").to_i - 10,l.index("#{needle}").to_i + 30] 
            else
                sn = l
            end
            sn = sn[0,60]
  ...
  begin    
    ...
    if /^(2|5)/.match(req.code.to_s) 
      ...
      if int_ips.size == 0
          ...
          if valid_ips > 0
             puts "[*] " + valid_ips.to_s + ' internal IP(s) found!'
    ...
    if $findall != 0
        ...
        if a1.size == 0
            puts "\n[x] no relevant IP/Domain(s) found"
            fout.puts("\n# [x] no relevant IP/Domain(s) found")
        end
        a1.uniq!
        a1.compact!
